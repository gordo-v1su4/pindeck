...
- **Images Convex module:** `api.images.*` is re-exported from [`convex/images/index.ts`](convex/images/index.ts). Domain logic lives in `library`, `ingest`, `moderation`, `lifecycle`, `uploads`, `analysis`, `generation`, and `shared` under [`convex/images/`](convex/images/). HTTP routes still import handlers from `./images`. Trigger orchestration lease/status writes live in [`convex/orchestrationState.ts`](convex/orchestrationState.ts); storage path helpers live in [`convex/lib/mediaAdapter.ts`](convex/lib/mediaAdapter.ts) (re-exported from [`convex/mediaAdapter.ts`](convex/mediaAdapter.ts)).
