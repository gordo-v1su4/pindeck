  ...
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      ...
      for (const quality of [0.82, 0.72, 0.62, 0.52]) {
        ...
        if (
          dataUrl.length <= ANALYSIS_IMAGE_MAX_DATA_URL_BYTES ||
          quality === 0.52
        ) {
          resolve(dataUrl);
          return;
...
async function pinterestRequest<T>(
  ...
  if (!response.ok) {
    throw new Error(
      body || `Pinterest ingest request failed (${response.status})`,
    );
  }
...
export function ImageUploadForm() {
  ...
  const previewUploadMetadata = useAction(
    (api as any).vision.previewUploadMetadata,
  );
  ...
  const discordPendingImages = (pendingImages || []).filter(
    (img) => img.sourceType === "discord",
  );
  const pinterestPendingImages = (pendingImages || []).filter(
    (img) => img.sourceType === "pinterest",
  );
  const localDraftImages = draftImages || [];
  const supportingGroups = (groups || []).filter(
    (group) => group.toLowerCase() !== "film",
  );
  const discordDraftImages = (draftImages || []).filter(
    (img) => img.sourceType === "discord",
  );
  const discordProcessingImages = (processingImages || []).filter(
    (img) => img.sourceType === "discord",
  );
  const pinterestDraftImages = (draftImages || []).filter(
    (img) => img.sourceType === "pinterest",
  );
  const pinterestProcessingImages = (processingImages || []).filter(
    (img) => img.sourceType === "pinterest",
  );
  ...
  >("local");
  const [pinterestSources, setPinterestSources] = useState<
    PinterestSourceRecord[]
  >([]);
  ...
  const [pinterestBusySourceId, setPinterestBusySourceId] = useState<
    string | null
  >(null);
  const [pinterestLastResult, setPinterestLastResult] = useState<string>("");

  // Discord queue image preview (click to enlarge)
  const [discordPreviewImageId, setDiscordPreviewImageId] =
    useState<Id<"images"> | null>(null);
  const discordPreviewImage =
    discordPendingImages.find((img) => img._id === discordPreviewImageId) ??
    null;

  // Post-upload variation modal state
  const [variationModalOpen, setVariationModalOpen] = useState(false);
  const [variationTargetImageIds, setVariationTargetImageIds] = useState<
    Id<"images">[]
  >([]);
  ...
  const hasPendingColorSampling = files.some(
    (file) => file.colorStatus === "pending",
  );
  const hasIncompleteColorSampling = files.some(
    (file) => file.colorStatus === "failed" || file.colors.length < 5,
  );
  const hasPendingMetadataAnalysis = files.some(
    (file) => file.metadataStatus === "pending",
  );

  useEffect(() => {
    void clearMyStaleProcessingImagesMutation({ olderThanHours: 18 }).catch(
      (error) => {
  ...
  const refreshPinterestSources = async () => {
    ...
    try {
      const sources =
        await pinterestRequest<PinterestSourceRecord[]>("/sources");
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to load Pinterest sources",
      );
    } finally {
  ...
  const savePinterestSource = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to save Pinterest source",
      );
    } finally {
  ...
  const runPinterestSource = async (sourceId: string) => {
    ...
    setPinterestLastResult("");
    try {
      const run = await pinterestRequest<PinterestRunRecord>(
        `/runs/${encodeURIComponent(sourceId)}`,
        {
          method: "POST",
        },
      );
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to run Pinterest source",
      );
    } finally {
  ...
  const syncPinterestSource = async (sourceId: string) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to sync Pinterest source",
      );
    } finally {
      setPinterestBusySourceId(null);
    }
        ...
        try {
          ...
          setFiles((prev) =>
            prev.map((file) => {
              if (file.id !== uploadFile.id) return file;
              const resolvedTitle =
                file.title || metadata.title || uploadFile.file.name;
              return {
                ...file,
                title: resolvedTitle,
                description:
                  file.description || metadata.description || file.description,
                ...
                projectName:
                  file.projectName || metadata.projectName || resolvedTitle,
                moodboardName:
                  file.moodboardName || metadata.moodboardName || undefined,
                uniqueId:
                  file.uniqueId || metadata.uniqueId || createUploadUniqueId(),
  ...
  const handleSubmit = async () => {
    ...
    if (hasPendingColorSampling) {
      toast.info(
        "Still sampling image colors. Upload will be ready in a moment.",
      );
      return;
    }
    ...
    if (hasPendingMetadataAnalysis) {
      toast.info(
        "Still generating metadata. Upload will be ready in a moment.",
      );
      return;
    }
        ...
        <>
          <Card className="p-6">
            ...
            {activeUploadTab === "discord" && (
              <Text size="2" color="gray">
                React with the configured emoji to queue RSS/Discord images.
                Review queued items below and keep or discard before they hit
                your main library.
              </Text>
            )}
            {activeUploadTab === "pinterest" && (
              <Text size="2" color="gray">
                Pinterest imports are staged for approval before analysis or
                publishing. Use the configured connector to send pins into this
                queue.
              </Text>
            )}
            {activeUploadTab === "automation" && (
              <Text size="2" color="gray">
                Scheduled board polling is the supported automation path for
                Pinterest board imports.
              </Text>
            )}
          </Card>
            ...
            <Box className="mt-6 animate-in fade-in">
              ...
              <Card className="p-4 mb-4">
                ...
                <Text size="2" color="gray" className="block mt-3">
                  The Discord bot imports into its configured{" "}
                  <code>PINDECK_USER_ID</code>. If that bot-side user ID differs
                  from the signed-in user shown here, this queue will stay empty
                  even when ingest succeeds.
                </Text>
              </Card>
              <Flex align="center" gap="2" className="mb-4">
                ...
                <Box>
                  ...
                  <Text size="2" color="gray">
                    {discordPendingImages.length} queued import
                    {discordPendingImages.length !== 1 ? "s" : ""}.
                  </Text>
                </Box>
              </Flex>
              ...
              {discordPendingImages.length === 0 ? (
                <Card className="p-4">
                  <Text size="2" color="gray">
                    No queued Discord imports yet. React to an RSS/Discord post
                    with your ingest emoji to queue it here.
                  </Text>
                </Card>
              ) : (
              ...
              {(discordProcessingImages.length > 0 ||
                discordDraftImages.length > 0) && (
                <Card className="p-4 mt-4">
                  <Flex align="center" justify="between" gap="3" wrap="wrap">
                    <Box>
                      ...
                      <Text size="2" color="gray">
                        {discordProcessingImages.length} processing,{" "}
                        {discordDraftImages.length} ready for review/finalize.
                      </Text>
                    </Box>
                    <Button
                      size="1"
              ...
              <Dialog.Root
                ...
                onOpenChange={(open) => {
                  if (!open) setDiscordPreviewImageId(null);
                }}
              >
                <Dialog.Content
                ...
                >
                  ...
                  <Dialog.Description className="sr-only">
                    Enlarged preview of queued Discord import
                  </Dialog.Description>
                  {discordPreviewImage && (
                    <Box className="flex flex-col">
                      <Box className="pd-glass-body relative flex items-center justify-center p-2">
                        <img
                          src={
                            discordPreviewImage.previewUrl ||
                            discordPreviewImage.imageUrl
                          }
                      ...
                      <Box className="pd-glass-footer p-3">
                        <Text weight="medium" size="2" className="block">
                          {(discordPreviewImage as any).title}
                        </Text>
                        <Text
                          size="2"
                          color="gray"
                          className="block mt-1 line-clamp-2"
                        >
                          {(discordPreviewImage as any).description}
                        </Text>
                      </Box>
                    </Box>
                  )}
          ...
          {activeUploadTab === "pinterest" && (
            <Box className="mt-6 animate-in fade-in">
                ...
                <Flex direction="column" gap="4">
                  <Flex align="start" justify="between" gap="3" wrap="wrap">
                    <Box>
                      ...
                      <Text size="2" color="gray" className="block">
                        Add a board, profile, or pin URL, then run it and send
                        new images into the review queue.
                      </Text>
                    </Box>
                    <Button
                      size="1"
                  ...
                  <Grid columns={{ initial: "1", md: "3" }} gap="3">
                    <Box className="md:col-span-2">
                      ...
                      <TextField.Root
                        ...
                        onChange={(event) =>
                          setPinterestSourceUrl(event.target.value)
                        }
                    ...
                    <Box>
                      ...
                      <TextField.Root
                        ...
                        onChange={(event) =>
                          setPinterestSourceName(event.target.value)
                        }
                    ...
                    <Box className="md:col-span-2">
                      ...
                      <TextField.Root
                        ...
                        onChange={(event) =>
                          setPinterestSourceTags(event.target.value)
                        }
                          ...
                          >
                            ...
                            <Flex gap="2" wrap="wrap">
                              <Button
                                ...
                                variant="soft"
                                onClick={() =>
                                  void runPinterestSource(source.id)
                                }
                              ...
                              <Button
                                ...
                                variant="soft"
                                onClick={() =>
                                  void syncPinterestSource(source.id)
                                }
                                disabled={pinterestBusySourceId === source.id}
                              >
                                Send to Queue
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  void runAndSyncPinterestSource(source.id)
                                }
                                disabled={pinterestBusySourceId === source.id}
                              >
                                Run + Send
              ...
              <Card className="p-4 mb-4">
                <Flex align="start" justify="between" gap="3" wrap="wrap">
                  <Box>
                    ...
                    <Text size="2" color="gray" className="block">
                      Queue query:{" "}
                      {discordQueueLoaded ? "Loaded" : "Loading..."}
                    </Text>
                  </Box>
                  <Flex gap="2" wrap="wrap">
                    <Badge color="gray" variant="soft">
              ...
              {(pinterestProcessingImages.length > 0 ||
                pinterestDraftImages.length > 0) && (
                <Card className="p-4 mt-4">
                  <Text size="2" weight="medium" className="block">
                    Approved Pinterest items moved out of queue
                  </Text>
                  <Text size="2" color="gray">
                    {pinterestProcessingImages.length} processing,{" "}
                    {pinterestDraftImages.length} ready for review/finalize.
                  </Text>
                </Card>
              )}
            </Box>
          ...
          >
            <Dialog.Content className="pd-glass-dialog !max-w-[min(95vw,32.5rem)] !p-0">
              <Box className="pd-glass-header px-5 py-4">
                ...
                <Dialog.Description
                  size="2"
                  className="mt-2 text-[var(--pd-ink-mute)]"
                >
                  Create AI-generated variations for{" "}
                  {variationTargetImageIds.length === 1
                    ? "this image"
                    : `${variationTargetImageIds.length} uploaded images`}
                  . Choose what kind of variations you want before anything gets
                  generated.
                </Dialog.Description>
              </Box>
                ...
                <Flex direction="column" gap="4">
                    ...
                    <Flex gap="2" wrap="wrap">
                      {COUNT_OPTIONS.map((count) => (
                        <button
                          ...
                          style={
                            variationCount === count ? chipSelected : chipBase
                          }
                        >
                          {count}
                        </button>
                  ...
                  <Box>
                    ...
                    <Text size="1" className="mt-1 text-[var(--pd-ink-faint)]">
                      None is the default; add a shot type only when you want to
                      steer framing.
                    </Text>
                  </Box>
                </Flex>
          ...
          >
            ...
            >
              ...
              <Text size="1" color="gray" className="max-w-xs">
                SVG, PNG, JPG or GIF (max. 10MB per file). Multiple files
                supported.
              </Text>
            </Flex>
          </Box>
          ...
          {files.length > 0 && (
            <Box className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                          ...
                          <Flex align="center" gap="2" className="mb-2 min-h-5">
                            {file.metadataStatus === "pending" && (
                              <>
                                ...
                                <Text size="1" color="gray">
                                  Generating metadata before upload...
                                </Text>
                              </>
                            )}
                            ...
                            {file.metadataStatus === "failed" && (
                              <>
                                ...
                                <Text size="1" color="gray">
                                  {file.metadataError ||
                                    "You can still upload and retry after."}
                                </Text>
                              </>
                            )}
                          </Flex>
                          ...
                          <Flex gap="1" wrap="wrap" className="mb-2">
                            {file.tags.map((tag, index) => (
                              <Badge
                                ...
                                style={getPaletteTagStyleForLabel(
                                  file.colors,
                                  tag,
                                  index,
                                )}
              ...
              <Flex
                justify="end"
                gap="3"
                className="pt-4 border-t border-gray-6"
              >
                <Button
                  variant="soft"
                  color="gray"
            ...
            <Box className="mt-8 animate-in fade-in">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    void Promise.all(
                      ...
                      .then(() =>
                        toast.success(
                          "All marked complete. Processing list cleared.",
                        ),
                      )
              ...
              <Grid columns={{ initial: "2", sm: "3", md: "4" }} gap="4">
                {processingImages.map((image) => (
                  <Card
                    key={image._id}
                    className="overflow-hidden opacity-80 p-2 group/card"
                  >
                    <Box className="relative aspect-video overflow-hidden bg-gray-100">
                      ...
                      >
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            void setAiStatusMutation({
                              imageId: image._id,
                              status: "completed",
                            })
                              .then(() =>
                                toast.success(
                                  "Marked complete. Image will leave the processing list.",
                                ),
                              )
                              .catch(() =>
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            ...
                            void setAiStatusMutation({
                              ...
                              .then(() =>
                                toast.info(
                                  "Marked failed. You can retry from the failed image.",
                                ),
                              )
                              .catch(() =>
                                toast.error("Failed to update status."),
              ...
              >
                <Flex align="center" gap="2">
                  ...
                  <Box>
                    ...
                    <Text size="2" color="gray">
                      {localDraftImages.length} image
                      {localDraftImages.length !== 1 ? "s" : ""} ready to
                      publish
                    </Text>
                  </Box>
                </Flex>
                    ...
                    <Flex gap="4">
                      ...
                      <Box className="flex-1 space-y-2 min-w-0">
                        <Flex justify="between" align="start" gap="2">
                          <Box className="flex-1">
                            <TextField.Root
                              value={image.title}
                              onChange={(e) =>
                                void updateImageMetadata(image._id, {
                        ...
                        <Box>
                          <Flex gap="1" wrap="wrap" className="mb-2">
                            {image.colors &&
                              sortColorsDarkToLight(image.colors).map(
                                (color) => (
                                  <Box
                                    key={color}
                          ...
                          <Flex gap="1" wrap="wrap" className="mb-2">
                            {image.tags.map((tag, index) => (
                              ...
                              >
                                ...
                                <button
                                  onClick={() =>
                                    void removeTagFromDraft(image._id, tag)
                                  }
                                  className="ml-1 text-current/70 transition-colors hover:text-current"
                                >
                                  <Cross2Icon width="10" height="10" />
                        ...
                        {image.aiStatus === "failed" && (
                          <Box className="space-y-2">
                            ...
                            <Text size="1" color="gray">
                              Check Convex logs for the exact failure. This can
                        ...
                        {image.aiStatus === "processing" && (
                          ...
                          >
                            <Box className="w-3 h-3 border-2 border-[var(--pd-green)] border-t-transparent rounded-full animate-spin" />
                            <Text size="1" color="gray">
                              Analyzing metadata and sampled colors...
                            </Text>
                          </Flex>
                        )}
                      </Box>
